<div align="center">

  <img
    src="https://capsule-render.vercel.app/api?type=waving&color=0:5b3f9d,100:358c97&height=200&section=header&text=You%20Live%20Once&fontColor=ffffff&fontSize=55&fontAlignY=38"
  />

  <h3>
    Backend & AI Service Developer
  </h3>

  <p>
    변화하는 기술을 배우고, 문제를 고민하고,<br>
    실제 서비스로 구현하는 개발자입니다.
  </p>

</div>

<br>

## 👋 About Me

- Backend 개발을 중심으로 웹 서비스와 AI 서비스를 개발하고 있습니다.
- Spring Boot와 Django를 활용한 REST API 및 서버 개발 경험이 있습니다.
- AI 모델 자체의 성능뿐만 아니라, 실제 서비스에서 AI를 어떻게 안정적으로 연결하고 운영할지에 관심이 있습니다.
- 팀 프로젝트와 해커톤에서 백엔드 개발 및 팀 리딩 경험을 쌓아왔습니다.

<br>

## 🏆 Awards & Activities

- 🥈 **2nd Place — Techeer 2025 Summer Bootcamp Hackathon**  
  `2025.08`

- 🤖 **LG Aimers 8기 AI 온라인 해커톤 팀장**  
  `91위 / 628팀 · 2026.02`

- 📊 **DACON × BDA 학습자 수료 예측 AI 경진대회 팀장**  
  `150위 / 275팀 · 2025.07 ~ 2025.08`

- 🎓 **LG Aimers 8기 수료**

- 📈 **BDA 10기 데이터 분석 / 전처리 과정 참여**

- 🌐 **GDG on Campus TUKorea 2025 운영총괄**

- 📱 **시흥시 CE 공모전 — Mommadang 팀장**  
  `Mobile Programming`

- 💻 **시흥시 CE 공모전 — Time-FIT 팀장**  
  `Web Service Programming`

<br>

## 🛠 Tech Stack

### Backend

<div align="center">

  <img src="https://img.shields.io/badge/Spring%20Boot-6DB33F?style=for-the-badge&logo=springboot&logoColor=white">
  <img src="https://img.shields.io/badge/Django-092E20?style=for-the-badge&logo=django&logoColor=white">
  <img src="https://img.shields.io/badge/REST%20API-005571?style=for-the-badge">

</div>

### Languages

<div align="center">

  <img src="https://img.shields.io/badge/Java-007396?style=for-the-badge&logo=openjdk&logoColor=white">
  <img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white">
  <img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black">
  <img src="https://img.shields.io/badge/C-A8B9CC?style=for-the-badge&logo=c&logoColor=black">

</div>

### AI & Data

<div align="center">

  <img src="https://img.shields.io/badge/PyTorch-EE4C2C?style=for-the-badge&logo=pytorch&logoColor=white">
  <img src="https://img.shields.io/badge/TensorFlow-FF6F00?style=for-the-badge&logo=tensorflow&logoColor=white">
  <img src="https://img.shields.io/badge/Pandas-150458?style=for-the-badge&logo=pandas&logoColor=white">

</div>

### Database & Infra

<div align="center">

  <img src="https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white">
  <img src="https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white">
  <img src="https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black">
  <img src="https://img.shields.io/badge/Firebase-FFCA28?style=for-the-badge&logo=firebase&logoColor=black">

</div>

### Mobile

<div align="center">

  <img src="https://img.shields.io/badge/Android-3DDC84?style=for-the-badge&logo=android&logoColor=white">

</div>

<br>

## 🚀 Projects

### 🎬 AI Museum Content Generation Service

> AI를 활용하여 박물관 콘텐츠를 자동 생성하는 서비스

- **Role:** Backend Developer
- **Team:** 5명
- **Period:** 6 Weeks
- **Result:** Techeer Bootcamp Hackathon 🥈 2nd Place
- Django REST 기반 어플리케이션 개발
- 비동기 영상 생성 파이프라인 구축
- 외부 AI API 연동 및 장애 상황을 고려한 fallback 처리

**Tech**

`Django REST` `Docker` `Celery` `Redis` `RabbitMQ` `asyncio` `OAuth2`

<br>

### ⏱ Time-FIT

> 사용자의 일정을 효율적으로 관리하기 위한 웹 서비스

- **Role:** Backend Developer / Team Leader
- REST API 설계 및 구현
- 인증 / 인가 기능 개발
- 데이터베이스 설계 및 서버 로직 구현
- 팀 프로젝트 전체 개발 과정 관리

**Tech**

`Spring Boot` `Java` `MySQL` `REST API`

<br>

### 🗺 SuperPoint-SLAM

> MobileNetV2 기반 SuperPoint 모델을 활용한 경량 SLAM 연구

- SuperPoint 기반 Feature Extraction 모델 경량화
- MobileNetV2 구조 활용
- 모델 가중치 양자화 및 경량화 실험
- **한국게임학회 논문 게재**

**Tech**

`Python` `PyTorch` `Deep Learning` `SLAM`

<br>

## 📫 Contact

<div align="center">

  <a href="https://github.com/Gro-J">
    <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white">
  </a>

  <a href="https://blog.naver.com/lucius_94">
    <img src="https://img.shields.io/badge/Naver%20Blog-03C75A?style=for-the-badge&logo=naver&logoColor=white">
  </a>

  <a href="mailto:uliveonce9892@gmail.com">
    <img src="https://img.shields.io/badge/Gmail-EA4335?style=for-the-badge&logo=gmail&logoColor=white">
  </a>

  <a href="https://www.instagram.com/z3r0gyxn/">
    <img src="https://img.shields.io/badge/Instagram-E4405F?style=for-the-badge&logo=instagram&logoColor=white">
  </a>

</div>

<br>

## 📊 GitHub Stats

<div align="center">

  <img
    src="https://github-readme-stats.vercel.app/api?username=Gro-J&show_icons=true&theme=tokyonight&hide_border=true"
    alt="Gro-J GitHub Stats"
  />

  <br><br>

  <img
    src="https://github-readme-stats.vercel.app/api/top-langs/?username=Gro-J&layout=compact&theme=tokyonight&hide_border=true"
    alt="Gro-J Top Languages"
  />

</div>
